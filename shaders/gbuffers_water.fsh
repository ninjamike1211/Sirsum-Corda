...
void main() {
	...
	if(blockType > 0.0 && blockType < 0.2) {
		...
		vec3 bumpmap = texture2D(colortex6, 0.125 * (horizontalPos + vec2(0.25, 0.75) * frameTimeCounter)).rgb - 0.5;
		bumpmap += texture2D(colortex9, 0.125 * (horizontalPos + vec2(0.4, -0.6) * frameTimeCounter)).rgb - 0.5;
		gl_FragData[1] = vec4((bumpmap * tbn) * 0.5f + 0.5f, 1.0);
		gl_FragData[2] = vec4(viewPos);
		gl_FragData[3] = vec4(lmcoord, 0.0, 1.0);
		gl_FragData[4] = vec4(0.75, 0.0, 1.0, 1.0);
