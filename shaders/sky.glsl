...
#define atmosphereRadiusMoon 5*637000
#define seaLevel 0
#define rayleighScaleHeight 7994
#define mieScaleHeight 1200
...
vec3 skyColor(vec3 viewDir, vec3 sunDir, float altitude, mat3 modelViewInverse) {
    ...
    vec3 result = 0.3 * (sumR * rayleighScatter * phaseR + sumM * mieScatter * phaseM) * mix(35, 220, sunDisk);
...
vec3 skyMoonColor(vec3 viewDir, vec3 sunDir, float altitude, mat3 modelViewInverse) {
    ...
    // We use a magic number here for the intensity of the sun (20). We will make it more
    // scientific in a future revision of this lesson/code
    vec3 result = (sumR * rayleighScatter * phaseR + sumM * mieScatter * phaseM) * 0.03 /* * vec3(0.2, 0.5, 1.0) */;
