...
void main() {
	...
	if (starData.a > 0.5) {
		...
		float sunDotV = max(dot(normalize(sunPosition), normalize(pos.xyz)), 0.0);
		...
		// if(gl_FragCoord.x < 960)
			color = sunAmount * sunColor + sunBlur * vec3(2.0, 2.0, 2.0) + max(1.0 - sunAmount - sunBlur, 0.0) * color;
