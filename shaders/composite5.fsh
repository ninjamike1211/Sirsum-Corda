...
void main() {
	vec3 current = texture2D(colortex0, texcoord).rgb;
		...
		current = (current + previousFrag) / 2.0;
	#endif

/* RENDERTARGETS: 0,15 */
	// gl_FragData[0] = vec4(velocity, 1.0);
	// gl_FragData[0] = texture2D(colortex0, prevScreenPos.xy);
	// gl_FragData[0] = vec4(vec2(mod(float(frameCounter), 2.0)), 0.0, 1.0);
	// gl_FragData[0] = vec4(decodeNormal(texture2D(colortex1, texcoord).rg*2.0-1.0), 1.0);
